...
static unsigned volatile was_send = 0, was_send_success = 0;
...
static void interrupt new_intr_handler(void) {
    if (was_send) {
        ...
        was_send_success = ret_code == 0xFA;
        was_send = 0;

        printf("0x%X\n", ret_code);
    } else {
        unsigned scancode = inportb(0x60);
        if (scancode == SCANCODE_ESC) do_run = 0;
    }
...
static void kb_send(unsigned char cmd) {
    ...
    for (kb_send_attempt_count = 0;
         kb_send_attempt_count < MAX_KB_SEND_ATTEMPT_COUNT;
         kb_send_attempt_count++) {
        while (inportb(0x64) & (1 << 1));
        outportb(0x60, cmd);

        was_send_success = 0, was_send = 1;
        while (was_send);
        if (was_send_success) return;
    }
